...
std::unique_ptr<ConnectionRoutingTable> ConnectionRoutingTable::buildFromGraphSnapshot(
    const std::vector<IExecutionSematicsProvider *> &components,
    const std::vector<Connection *> &connections)
{
    auto routingTable = std::make_unique<ConnectionRoutingTable>();

    // TODO: Build the routing table based on the components and connections provided.

    return routingTable;
}

void TokenRouter::onTokenProduced(const QString& sourceId,
                                  const QVariantMap& outputState,
                                  const ConnectionRoutingTable& routingTable)
{
    // TODO: Implement token routing logic based on the routing table.
}

QVariantMap TokenRouter::consumeTokensFor(const QString& componentId)
{
    QVariantMap mergedTokens;
    // TODO: Implement logic to merge tokens for the specified componentId from the tokenQueue_.
    return mergedTokens;
}


ActorScheduler::ActorScheduler(ActorProcessingMessageFinishedEvent finishedEvent, size_t maxThreads)
    : workers_(maxThreads)
    , tokenRouter_(std::make_unique<TokenRouter>())
    , externalCallback_(finishedEvent)
{
